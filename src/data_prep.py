...
def prepare_data() -> pd.DataFrame:
    ...
    for stat in stats_list:
        team_stats[stat] = (
            ...
            .apply(lambda x: x.ewm(alpha=alph, adjust=False).mean())
    ...
    team_stats["wr"] = (
        ...
        .sort_values('year')
        .groupby(by=['tmID'])["wr"]
        .apply(lambda x: x.ewm(alpha=alph, adjust=False).mean()) # Alpha maior = mais peso para os valores mais recentes | Adjust faria os valores serem normalizados
        .reset_index(level=0, drop=True)
    )
